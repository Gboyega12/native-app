...
interface AchievementContext {
  ...
  allMoves: Array<{ monthlyImpact?: number }>;
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const results = { sent: 0, silenced: 0, skipped: 0, failed: 0, errors: [] as Array<{ user_id: string; key?: string; error: string }> };
  ...
  try {
    ...
    const byUser: Record<string, Array<{ id: string; user_id: string; achievement_key: string; unlocked_at: string }>> = {};
      ...
      try {
        ...
        for (const ach of valuable) {
          try {
            ...
            const message = achErr instanceof Error ? achErr.message : String(achErr);
            results.failed++;
            results.errors.push({ user_id: userId, key: ach.achievement_key, error: message });
    ...
    console.error('[achievement-alerts] Cron failed:', message);
...
function buildAchievementMessage(key: string, ctx: AchievementContext): AchievementMessage | null {
...
function buildAchievementHtml(msg: AchievementMessage, name: string): string {
  const BRAND = '#00d4aa';
  const BG = '#0A0A0A';
