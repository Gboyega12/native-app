...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    console.error('[stt] Transcription error:', message);
    ...
    if (DEEPGRAM_API_KEY && OPENAI_API_KEY) {
      try {
        ...
        console.error('[stt] Whisper fallback also failed:', fallbackMessage);
      }
    }
...
// ── Deepgram Nova-2 ──
async function transcribeDeepgram(audioBuffer: Buffer, contentType: string): Promise<string> {
...
// ── OpenAI Whisper ──
async function transcribeWhisper(audioBuffer: Buffer, contentType: string): Promise<string> {
